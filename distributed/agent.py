...
class Agent:
	...
	# Simulated Annealing that only jump if better energy
	
	def simulated_annealing_bjump(self, ls_prob_ss, fact_ls, prob_jump, radius_jump, temp_init, hist):
		# LS only in agent's self.current pocket
		for i in range (1, len(self.current.pose.sequence())):
			...
			if (i+1) != len(self.current.pose.sequence()):
				...
				if random.random() <= ls_prob_ss[self.sequence.secondary_sequence_list[i]]:
					...
					while(temperature > 0.1):
						...
						phi += bit
						if (phi > self.point_center(phi) - 0.5) and (phi < self.point_center(phi) + 0.5):
							# Alteracao temporaria
							self.current.pose.set_phi(i+1, phi)
							self.current.calculate_energy()
							energy_temp = self.current.energy_value
	...
	# Simulated Annealing that phi psi coordinates at the same time
	
	def simulated_annealing_phipsi(self, ls_prob_ss, fact_ls, prob_jump, radius_jump, temp_init, hist):
		# LS only in agent's self.current pocket
		for i in range (1, len(self.current.pose.sequence())):
			...
			if (i+1) != len(self.current.pose.sequence()):
				...
				if random.random() <= ls_prob_ss[self.sequence.secondary_sequence_list[i]]:
					...
					while(temperature > 0.1):
						...
						if (phi > self.point_center(backup_phi) - 0.5) and (phi < self.point_center(backup_phi) + 0.5) and (psi > self.point_center(backup_psi) - 0.5) and (psi < self.point_center(backup_psi) + 0.5):
							# Alteracao temporaria
							self.current.pose.set_phi(i+1, phi)
							self.current.pose.set_psi(i+1, psi)
							self.current.calculate_energy()
	...
	# Simulated Annealing that explore and modify the position of its radius
	
	def simulated_annealing_mobile(self, ls_prob_ss, fact_ls, prob_jump, radius_jump, temp_init, hist):
		# LS only in agent's self.current pocket
		for i in range (1, len(self.current.pose.sequence())):
	...
	# Simulated Annealing that combines all the above implemented
